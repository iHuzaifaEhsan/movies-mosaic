    ...
    <div className='flex flex-wrap w=[full] h-[full] mt-5'>
      {data.map((c, i) =>
        <Link to={`/${c.media_type || title}/details/${c.id}`} className='relative w-[25vh] mr-[5%] mb-[5%] position-relative' key={i}>
          <img className='h-[40vh] object-cover shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)]' src={c.poster_path || c.backdrop_path || c.profile_path || c.logo_path || c.still_path ? `https://image.tmdb.org/t/p/original/${c.poster_path || c.backdrop_path || c.profile_path || c.logo_path || c.still_path}` : noimage} alt="" />
